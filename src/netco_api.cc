...
void netco::co_go(std::function<void()> &&func, Socket *socket, size_t stackSize, int tid)
{
	if (tid < 0)
	{
		netco::Scheduler::getScheduler()->createNewCo(std::move(func), socket, stackSize);
	}
	else
	{
		tid %= netco::Scheduler::getScheduler()->getProCnt();
		netco::Scheduler::getScheduler()->getProcessor(tid)->goNewCo(std::move(func), socket, stackSize);
	}
}
